...
class FacebookRequest(BaseModel):
    subject: str = Field(..., min_length=1)
    type: Literal["post", "event", "publicite"]
    language: Literal["fr", "en"]
...
_SYSTEM = (
    "Vous etes un assistant expert en generation de contenu professionnel. "
    "Vous aidez a produire des textes clairs, engageants et adaptes au format demande. "
    "Respectez le ton, la langue, et fournissez toujours des hashtags ou appels a action pertinents. "
    "Repondez uniquement avec le contenu demande et une liste de suggestions."
)
...
async def generate_linkedin(body: LinkedInRequest):
    prompt = (
        ...
        "des hashtags et un appel a action clair. Rends le texte professionnel et accessible."
    )
    try:
        content = await _generate_text(prompt)
        return ContentResponse(content=content, suggestions=[
            "Variez les hashtags pour toucher une audience plus large",
            "Ajoutez une question finale pour ameliorer engagement",
...
async def generate_newsletter(body: NewsletterRequest):
    prompt = (
        ...
        "Ajoute des conseils actionnables, un ton professionnel et un appel a action a la fin."
    )
    try:
        content = await _generate_text(prompt)
        return ContentResponse(content=content, suggestions=[
            "Personnalisez introduction avec le nom du destinataire",
            "Incluez un lien vers une ressource ou un article lie",
...
async def generate_email(body: EmailRequest):
    type_map = {
        ...
        "remerciement": "un email de remerciement apres un rendez-vous ou une collaboration",
    }
    prompt = (
        f"Redige {type_map[body.type]} en {body.language} sur le contexte suivant : {body.context}. "
        "Sois clair, structure et termine avec un appel a action simple."
    )
    try:
        content = await _generate_text(prompt)
        return ContentResponse(content=content, suggestions=[
            "Personnalisez objet pour augmenter le taux ouverture",
            "Ajoutez un paragraphe de preuve sociale ou de resultats",
            "Incluez une phrase de relance polie en fin email",
        ])
    except Exception as exc:
        raise HTTPException(status_code=500, detail=str(exc))
...
async def generate_instagram(body: InstagramRequest):
    ...
    try:
        ...
        return ContentResponse(content=content, suggestions=[
            ...
            "Ajoute un appel a action clair",
...
async def generate_facebook(body: FacebookRequest):
    ...
    try:
        ...
        return ContentResponse(content=content, suggestions=[
            ...
            "Precise le public cible pour ameliorer la diffusion",
        ])
    except Exception as exc:
        raise HTTPException(status_code=500, detail=str(exc))
...
async def generate_blog(body: BlogRequest):
    ...
    try:
        content = await _generate_text(prompt)
        return ContentResponse(content=content, suggestions=[
            "Structure article avec des sous-titres clairs",
            "Inclue des mots-cles naturellement",
            "Ajoute un CTA pertinent en fin article",
        ])
    except Exception as exc:
        raise HTTPException(status_code=500, detail=str(exc))
